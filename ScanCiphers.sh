...
function print_help(){
	...
	print_help_line  "-s:	Destination server and port seperated by colon. Deafult is localhost:443"
...
function init_ciphers_array(){
	...
	ciphers_cmd="openssl ciphers '$CIPHER_SUITE'"
...
function init_ssl_array(){
	...
	# so the array should be initialized with the openssl ssl/tls 
	# version format
	if [ "$GET" == "false" ];then
		case "$TLS_VERSION" in
...
function test_ciphers(){
	...
	for CIPHER in ${CIPHERS[@]}; do
		...
		if [ "$GET" == "false" ]; then
			res=$(echo -e "GET / HTTP/1.1\r\nConnection: Close\r\n\r\n" | openssl s_client -cipher "$CIPHER" "$SSL_VERSION" -connect "$SERVER_ADDRESS":"$SERVER_PORT" -servername "$SERVER_ADDRESS" 2>&1)
		else
...
init_ciphers_array
echo
print_help_header "Obtaining cipher list from $(openssl version)."
test_versions
